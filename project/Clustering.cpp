...
//END DEBUG
		...
		int widthFinal = min(WIDHT_BIN/2, img->cols - meanInit.x)*2;
		int heightFinal = min(HEIGHT_BIN/2, img->rows - meanInit.y)*2;
...
//			iterMatching = peopleMatching.erase(iterMatching);
		...
		else
		{
			...
			xInit = max(0,col-1);
			yInit = max(0, row-1);
			w = min(clusterImg.cols-xInit, 3);
			h = min(clusterImg.rows-yInit, 3);
